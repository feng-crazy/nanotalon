...
func TestToolsFunctionality(t *testing.T) {
	...
	writeParams := map[string]interface{}{
		...
		"content": "Written by nanotalon functional test!",
	}
	...
	if err != nil {
		...
		} else if string(content) != "Written by nanotalon functional test!" {
			t.Errorf("File content mismatch: got %s, want %s", string(content), "Written by nanotalon functional test!")
		} else {
