...
func TestCLICommands(t *testing.T) {
	...
	tempDir := t.TempDir()
	binaryPath := filepath.Join(tempDir, "nanotalon-test")

	// Build the binary using go build (use the correct path)
	buildCmd := exec.Command("go", "build", "-o", binaryPath, "nanotalon/cmd/main.go")
	buildOutput, err := buildCmd.CombinedOutput()
	if err != nil {
		t.Fatalf("Failed to build nanotalon: %v\nOutput: %s", err, string(buildOutput))
	}

	t.Logf("✓ Built nanotalon binary at: %s", binaryPath)
	...
	if !strings.Contains(helpStr, "nanotalon") || !strings.Contains(helpStr, "Personal AI Assistant") {
